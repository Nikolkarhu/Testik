...
def sleep():
        ...
        if Abobus == 2:
            print ("Ты не спал так долго что тебе померещился ОГРОМНЫЙ и добрый великан, который отвёл тебя в лес.")
            energy -= 30
            walk()
            return
        for zzz in range(5):
...
def food():
        ...
        if food_suply <= 30:
          print ("НАШИ ЗАПАСЫ ГРЕЧКИ И ПЕЛЬМЕНЕЙ ЗАКОНЧИЛИСЬ!!!!!!!!!!!")
          return
        elif energy > 99:
...
def walk():
    ...
    if napadaem == 1:
          ...
          print ("Я НАШЁЛ ОГРОМНОЕ НЕИЗВЕСТНОЕ ЗЕЛЬЕ. ПИТЬ ИЛИ НЕТ 1 = Пить 2 = нет")
          pit = input ("Ответ:")
          if pit == 1:
                Effect = random.randint (1,3)
                if Effect == 1:
